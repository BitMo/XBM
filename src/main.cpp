...
// Lower values to make sure chain moves
// fast staking, smaller amounts
unsigned int nStakeMinAge = (60 * 15); // 15 minutes
unsigned int nStakeMaxAge = (60 * 60); // 60 minutes

unsigned int nModifierInterval = 30; // time to elapse before new modifier is computed
...
const string strMessageMagic = "BitMo Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // BitMo: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // BitMo: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    // 50 Blocks for movement. Everything else is sorted. Boom
    if (pindexBest->nHeight == 0)       // C-Cex ICO + 0.45 for Fees
        nSubsidy = 3270177.48307694;
    else if (pindexBest->nHeight == 1)  // Escrow ICO, 1,473,964 XBM +0.45 for Fees
        nSubsidy = 1473964.45;
    else if (pindexBest->nHeight < 201) // Unused coin, used for generating blocks only.
        nSubsidy = 1;
...
{
    // Cutoff point for PoS. Return nFees for PoS to keep us green
    // 5 years + premine blocks.
    if (pindexBest->nHeight >= 5256549)
        return nFees;
...
// BitMo: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes BitMo fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // BitMo: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // BitMo: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // BitMo: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // BitMo: track money supply and mint amount info
...
// BitMo: total coin age spent in transaction, in the unit of coin-days.
...
// BitMo: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // BitMo: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // BitMo: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // BitMo: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // BitMo: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // BitMo: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // BitMo: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // BitMo: getblocks may not obtain the ancestor block rejected
    ...
    // BitMo: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitMo", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "26.8.2014 BitMo will stand throughout the ages";
        CTransaction txNew;
        txNew.nTime = 1409063212;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1409063212;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1105468;
        if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xab8cf96762dd6d408f54689cc0b6fdb1be0fbebbcf1fc4e5e7d2f41b2bbc98b3"));
        ...
        // BitMo: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x41, 0x63, 0x69, 0x64 };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
		...
		// For users to use same version. Prevent issues.
