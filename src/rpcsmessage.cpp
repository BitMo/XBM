...
// Copyright (c) 2014 The BitMo developers
...
Value bitmessage(const Array& params, bool fHelp)
{
    ...
    if (fSecMsgEnabled)
        throw runtime_error("bitmessage is already enabled.");
    
    Object result;
    if (!SecureMsgEnable())
    {
        result.push_back(Pair("result", "Failed to enable bitmessage"));
    } else
    {
        result.push_back(Pair("result", "bitmessage enabled."));
    }
    return result;
}

Value bitdisable(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "bitdisable \n"
            "Disable bitmessage");
    if (!fSecMsgEnabled)
        throw runtime_error("bitmessage is already disabled.");
    
    Object result;
    if (!SecureMsgDisable())
    {
        result.push_back(Pair("result", "Failed to disable bitmessage."));
    } else
    {
        result.push_back(Pair("result", "bitmessage disabled."));
    }
    return result;
}
