...
static const int LAST_POW_BLOCK = 500; // Should be enough to keep PoS staking.
...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.10 * COIN;	// 10% yearly interest
...
static const uint256 hashGenesisBlock("0x00000ed81f46a206affb746e99ea1b7c38acc717a88eab7fb6071d761544c851");
...
public:
    ...
    {
        // BitMo: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // BitMo: get transaction coin age
...
public:
    ...
    // BitMo: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // BitMo: two types of block: proof-of-work or proof-of-stake
    ...
    // BitMo: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // BitMo: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // BitMo: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // BitMo: block index flags
    enum  
    {
