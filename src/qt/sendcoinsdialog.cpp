    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a BitMo address (e.g. Sjz75uKHzUQJnSdzvpiigEGxseKkDhQToX)"));
#endif
...
void SendCoinsDialog::on_sendButton_clicked()
{
   ...
   // check distmix checkbox
    ...
    if (ui->checkDistmix->isChecked())
    {
        ...
        const std::string distmixCommand("bitsend");
        try
        {
            foreach(const SendCoinsRecipient &rcp, recipients)
            {
                std::vector<std::string> params;
                //requires Wallet called mixer
                params.push_back("mixer");
                params.push_back(rcp.address.toStdString());
                params.push_back(boost::lexical_cast<std::string>((double)rcp.amount/COIN));
                // no multipath on this request - final loop
        ...
        else
        {
            QMessageBox::warning(this, tr("Send Coins"),
                trUtf8("Failed, code %1\n").arg(QString::number(errCode), QString::fromStdString(errMessage)),
                QMessageBox::Ok, QMessageBox::Ok);
            sendstatus.status = WalletModel::Aborted;
        }
    }
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid BitMo address"));
        }
        else
        {
